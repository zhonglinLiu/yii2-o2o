...
CREATE TABLE `o2o_deal`(
  ...
  `location_ids` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '参加活动地址',
 ...
 */
 CREATE TABLE `o2o_order`(
  ...
  `transcation_id` VARCHAR(100) NOT NULL default '' COMMENT '支付编号',
  `user_id` int(11) unsigned NOT NULL default 0,
  `username` VARCHAR(50) NOT NULL default '',
  `pay_time` VARCHAR(20) NOT NULL default 0 COMMENT '支付时间精确到毫秒',
  `payment_id` tinyint(1) unsigned NOT NULL default 1 COMMENT '支付方式',
  `deal_id` int(11) unsigned NOT NULL default 0 COMMENT '商品id',
  `deal_count` smallint unsigned NOT NULL default 1,
  `pay_status` tinyint(1) unsigned NOT NULL default 1 COMMENT '支付状态 0未支付1支付成功2支付失败',
  `total_price` decimal(10,2) NOT NULL default '0.00' COMMENT '总价',
  `pay_amount` decimal(10,2) NOT NULL default '0.00' COMMENT '支付机构返回的总额',
  `referer` VARCHAR(100) NOT NULL default '' COMMENT '订单来源$_SERVER["HTTP_REFERER"]',
  `status` tinyint(1) NOT NULL DEFAULT 0,
  `create_time` int(11) unsigned NOT NULL default 0,
  `update_time` int(11) unsigned NOT NULL default 0,
  PRIMARY KEY (`id`),
